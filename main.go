...
func main() {
	...
	if optsInOut.HexDump {
		ioc = &inout.HexDump{ioc}
	}
	if optsInOut.Interval > 0 {
		ioc = &inout.Interval{ioc, optsInOut.Interval}
	}
	...
	// Actions
	if optsAction.Detect {
		action.NewRaiseTTY(s).Register()
	}
